...
| `get_open_work` | open tickets for the property, client-safe |
...
Tools live in `webhook-server/skills/mcp_context.py` (Layer 2). The protocol
...
- `get_spend_authorization` builds the portfolio spend sheet on a cold call
  (~750 rows) and `get_leasing_funnel` queries BigQuery. Both inherit the
