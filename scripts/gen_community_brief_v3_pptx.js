/* Generates a coworker briefing deck for the Community Brief —
 * how it works + what triggers it. RPM brand, numbered-step motif.
 * Reflects the current (v3) schema + trigger logic.
 * Run: node scripts/gen_community_brief_v3_pptx.js
 * Output: ~/Downloads/RPM_Community_Brief.pptx
 */
...
pres.title = "RPM Community Brief — How It Works";
const W = 13.33, H = 7.5;

const sh = () => ({ type: "outer", color: "000000", blur: 7, offset: 3, angle: 135, opacity: 0.13 });

function footer(slide, n) {
  slide.addText("RPM Community Brief — how it works & what triggers it",
...
// ════ SLIDE 3 — Before → After (light, two columns) ════
{
  ...
  s.addText("NOW", { x: 7.15, y: 2.0, w: 5.2, h: 0.6, valign: "middle", fontFace: BF, fontSize: 14,
    bold: true, color: WHITE, charSpacing: 2, margin: 0 });
  s.addText([
    "Multi-select Voice Tier + Unit Noun (apartment → penthouse)",
    "Romance Paragraph + Unit-Level Details, written in",
    "Structured floor plans from Apt IQ (name/beds/baths/sq ft)",
    "Property Amenities split from In-Unit Features",
    "Tracking # + UTM for all 13 sources",
    "Documents: pitch decks, RFPs, brand guides",
    "Editable on BOTH the approval link and /accounts",
    "Auto-captured daily for every managed property w/ a deal",
  ].map((t) => ({ text: t, options: { bullet: { code: "2022", indent: 14 }, breakLine: true, paraSpaceAfter: 7 } })),
    { x: 7.15, y: 2.85, w: 5.2, h: 3.6, fontFace: BF, fontSize: 13.5, color: INK, valign: "top" });
  footer(s, 3);
}
...
// ════ SLIDE 4 — The structure (light, 3 panels) ════
{
  ...
  const cols = [
    [["Voice & Positioning", "Multi-select tier + unit noun, names, former name"],
     ["Brand & Story", "Taglines, differentiators, romance paragraph, residents"],
     ["Lifecycle", "Lease-up / stabilized / renovated"],
     ["Inventory", "Apt IQ floor plans + unit-level details & sq ft"]],
...
// ════ SLIDE 5 — End-to-end flow (hero diagram, light) ════
{
  ...
  const stages = [
    ["Managed + has deal", "daily trigger"],
...
stepSlide(1, 6, "It starts itself", "Auto-capture — daily, no ticket needed",
  [ "A daily cron scans HubSpot for every actively-managed property that has a deal — no one has to kick it off.",
    "For each, it scrapes the property site + drafts the brief — capturing what it can, automatically.",
    "The approval link is written ONTO the company record so it never gets lost." ],
  "AUTO-CAPTURE", [
    "daily cron scan",
    "scope: plestatus NOT IN",
    "  {Disposition Complete,",
    "   Management Not Awarded}",
    "  AND ≥ 1 deal",
    "",
    "→ scrape + LLM draft",
    "→ brief + approval link",
...
{
  const s = pres.addSlide();
  lightHeader(s, "Where it stands", "Live today — and how the team uses it", 26);
  // Live card
  s.addShape(pres.shapes.RECTANGLE, { x: 0.7, y: 2.0, w: 5.9, h: 4.6, fill: { color: CARD },
    line: { color: CARDLINE, width: 1 }, shadow: sh() });
  s.addShape(pres.shapes.RECTANGLE, { x: 0.7, y: 2.0, w: 5.9, h: 0.6, fill: { color: SAGE } });
  s.addText("LIVE & RUNNING", { x: 0.95, y: 2.0, w: 5.4, h: 0.6, valign: "middle", fontFace: BF,
    fontSize: 14, bold: true, color: WHITE, charSpacing: 1.5, margin: 0 });
  s.addText([
    "Daily auto-capture + 30-day Apt IQ retry",
    "Structured floor plans from Apt IQ",
    "Multi-select Voice Tier + Unit Noun",
    "Romance Paragraph + Unit-Level Details",
    "Editable approval portal + /accounts/property",
    "ClickUp attestation gate; override-wins everywhere",
  ].map((t) => ({ text: t, options: { bullet: { code: "2713", indent: 14 }, breakLine: true, paraSpaceAfter: 10 } })),
    { x: 0.95, y: 2.85, w: 5.4, h: 3.6, fontFace: BF, fontSize: 14, color: INK, valign: "top" });
  // How-to card
  s.addShape(pres.shapes.RECTANGLE, { x: 6.9, y: 2.0, w: 5.7, h: 4.6, fill: { color: CARD },
    line: { color: CARDLINE, width: 1 }, shadow: sh() });
  s.addShape(pres.shapes.RECTANGLE, { x: 6.9, y: 2.0, w: 5.7, h: 0.6, fill: { color: COPPER } });
  s.addText("HOW THE TEAM USES IT", { x: 7.15, y: 2.0, w: 5.2, h: 0.6, valign: "middle", fontFace: BF,
    fontSize: 14, bold: true, color: WHITE, charSpacing: 1.5, margin: 0 });
  s.addText([
    "Open the approval link on the company — or edit on /accounts/property",
    "Confirm what's right, fix what isn't — inline",
    "Fill the tracking numbers; attach decks / RFPs",
    "Check the attestation box in ClickUp",
    "Edits publish + Fluency re-reads daily — no re-trigger needed",
  ].map((t) => ({ text: t, options: { bullet: { code: "2022", indent: 14 }, breakLine: true, paraSpaceAfter: 11 } })),
    { x: 7.15, y: 2.85, w: 5.2, h: 3.6, fontFace: BF, fontSize: 14, color: INK, valign: "top" });
  footer(s, 11);
}
...
// ════ SLIDE 12 — Takeaway (dark) ════
{
  ...
  s.addText([
    { text: "Auto-captured daily for every managed property with a deal — then steered by Property Marketing.", options: { bullet: { code: "2022", indent: 14 }, color: "E6EDF7", breakLine: true, paraSpaceAfter: 11 } },
  ...
  s.addText("Full detail: docs/CLIENT_BRIEF_SYSTEM.md  ·  “Community Brief”",
    { x: 0.9, y: 6.85, w: 11.6, h: 0.4, fontFace: BF, fontSize: 12, color: "9AA7B4", margin: 0 });
}

const out = path.join(os.homedir(), "Downloads", "RPM_Community_Brief.pptx");
