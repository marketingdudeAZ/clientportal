...
SYSTEM_PROMPT = (
    ...
    "aware (paid search, paid social, SEO/organic, display, etc.). Frame the "
    "quadrants correctly: STRENGTHS and WEAKNESSES are INTERNAL — things RPM "
    "controls on this campaign (budget mix, channel coverage, creative, funnel "
    "efficiency, CPL). OPPORTUNITIES and THREATS are EXTERNAL — the market and "
    "competitors (demand, competitor pricing/concessions, the property's rent vs "
    "comps, reputation vs comps, seasonality). Note when spending more will NOT "
    "fix an external problem (e.g. priced above the market). Each item is "
