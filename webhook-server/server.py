...
def video_approve():
    ...
    body_out, status = approve_video_variants(company_id, variant_ids)
...
def _callprep_update_rec(company_id: str, rec_id: str, new_status: str, email: str) -> dict | None:
    ...
    Returns the updated payload (or None on failure). The implementation lives
    in approval_actions so the workspace API can run it as a module call.
    """
    from approval_actions import callprep_update_rec
    return callprep_update_rec(company_id, rec_id, new_status, email)
...
def callprep_approve():
    ...
    from approval_actions import callprep_approve as _approve_callprep_rec
    result, _clickup = _approve_callprep_rec(company_id, rec_id, email)
    if not result:
        return jsonify({"error": "Recommendation not found or update failed"}), 404
